    ...
    <div class="w-full max-w-4xl mx-auto flex justify-between items-center mt-4 px-4">
        <h1 class="font-bold text-blue-600 text-[20px] flex items-center">
            <a href="{{ route('index') }}" class="flex items-center">
                E- <p class="text-[#000000]">Wallet-Groupe</p>
...
<section class="w-full max-w-4xl mx-auto mt-8 px-4 flex-1">
    <div class="shadow-xl p-8 md:p-16 rounded-lg bg-white flex flex-col items-center">
        <h1 class="text-[35px] font-bold mb-8 text-center">{{ $groupe->balance }}{{ $groupe->devise }}</h1>
        ...
        <div class="w-full flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-4">
            <div class="cursor-pointer p-3 shadow-md bg-blue-600 text-white rounded-md w-full md:w-1/4 text-center">
                <p class="flex items-center justify-center">
                    ...
                    <a href="{{ route('showentreegroupe') }}">Entrée argent</a>
                </p>
            </div>
            <div class="cursor-pointer p-3 shadow-md bg-blue-600 text-white rounded-md w-full md:w-1/4 text-center">
                <p class="flex items-center justify-center">
                    <img src="{{ asset('images/silhouette-dutilisateurs-multiples.png') }}" alt="" class="w-5 mr-2 mt-1">
                    <a href="{{ route('showprofilegroupe') }}">Profil Groupe</a>
                </p>
            </div>
...
<section class="w-full max-w-4xl mx-auto mt-8 px-4 flex flex-col md:flex-row justify-between items-center">
    ...
    <form action="{{ route('resetTransactiongroupe') }}" method="POST">
        @csrf
        @method('DELETE')
    ...
    @foreach ($transactionGroupe as $transactionGroupes)
    ...
    <div class="flex items-center">
        ...
        <div class="md:ml-4">
            <h2 class="font-bold ml-1">{{ $transactionGroupes->name }}</h2>
            <h4 class="text-sm text-gray-500 ml-1">{{ $transactionGroupes->created_at->format('F Y à H:i') }}</h4>
        </div>
    </div>
    ...
    <div class="flex items-center space-x-4">
        ...
        <form action="{{ route('deleteTransaction', $transactionGroupes->id) }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="submit" class="cursor-pointer">
